...
\begin{table}[hbt]
	...
	\begin{tabular}{rc}
		\toprule
		\textbf{Charge} & \textbf{Bunch Length RMS}   \\
		\midrule
		\SI{0.3}{nC} & \SI{2.2}{ps}     \\ %[3pt]
		\SI{0.7}{nC} & \SI{2.6}{ps}     \\ %[3pt]
		\SI{1.3}{nC} & \SI{2.6}{ps}      \\
		\SI{40}{nC}  & \SI{6}{ps}   \\ %[3pt]
		\bottomrule
	\end{tabular}
	\label{exp}
\end{table}
...
\begin{figure}
	\includegraphics[width=1.0\linewidth]{images/bunchlength}
	\caption{Comparison of simulation and experimental measurements.}
	\label{sims}
\end{figure}

\section{Conclusion}
Simulations of an AWA photoinjector were done in OPAL.
Experimental measurements of the bunch length were done using CTR.
The data was analyzed and compared to simulation results.
The bunch length for 1 and \SI{40}{nC} is reported.
There is good agreement between the simulations and 
experimental measurements. This method can now be used
in future experiments at the AWA to measure bunch length.
